...
## 列表 / 表格（后台）—— 平台内所有后台列表统一遵守
- **列表用表格形式 + 行内绝不放"操作"按钮**：整行可点击 → 进入**详情页**，所有操作（通过/驳回/编辑/删除/启用停用等）一律放在详情页内。这是平台**全局规范**，所有后台列表都照此办。
- 表格列：用 `grid` 对齐多列（姓名/编号/时间/状态…），行是 `<Link>` 到详情；末列放状态 + `ChevronRight`；移动端可降列只留主信息。`DataTable` 用 `dropActionCol` 砍掉"操作"列。
- 参考实现：协会「会员审核 / 报备审批 / 调解 / 用户管理」列表 + 对应 `[id]`（或 `[phone]`）详情页；企业「客户线索」列表 + `leads/[id]` 详情（含状态流转：新→沟通→量房→签单/流失）。
