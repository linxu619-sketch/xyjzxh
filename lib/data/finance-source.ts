...
export type FinanceProduct = {
  ...
  rateLabel: string; amountLabel: string; termLabel: string; forWhom: string; color: string; status: string; createdAt: number;
};
...
type PRow = { id: number; name: string | null; provider: string | null; type: string | null; rate_label: string | null; amount_label: string | null; term_label: string | null; for_whom: string | null; color: string | null; status: string; created_at: number | null };
...
function toP(r: PRow): FinanceProduct {
  return { id: r.id, name: r.name ?? "", provider: r.provider ?? "", type: r.type ?? "", rateLabel: r.rate_label ?? "", amountLabel: r.amount_label ?? "", termLabel: r.term_label ?? "", forWhom: r.for_whom ?? "", color: r.color ?? "brand", status: r.status ?? "active", createdAt: r.created_at ?? 0 };
}
