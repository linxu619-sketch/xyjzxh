...
import { MapPin, Sparkles, ChevronRight, ShieldCheck, ArrowUpRight, CheckCircle2, SlidersHorizontal, GraduationCap } from "lucide-react";
...
import { listOpenHires, listApplicationsByPractitioner } from "@/lib/data/jobs";
...
import { applyHireAction } from "./actions";
import { effectivePractitionerPhone, isPractitionerPreview } from "@/lib/dashboard/preview";

export const metadata = { title: "找工作 · 从业者门户" };

const STATUS_LABEL: Record<string, string> = { pending: "已投递 · 待企业处理", accepted: "已被录用 🎉", rejected: "未通过" };

export default async function PractitionerHire({ searchParams }: { searchParams: Promise<{ aok?: string; adup?: string; aerr?: string; pv?: string; all?: string }> }) {
  ...
  const showAll = all === "1";
  const infoIncomplete = !me?.expectMonthMin;

  return (
    <PractitionerShell title="找工作" subtitle={`长期岗位 · 月薪 · 适配 ${matched.length} 个 · 共 ${matched.length + others.length} 个在招`}>
      <JobHireSwitcher active="hire" />

      {pv && <Banner>预览态为只读体验，操作未提交。以从业者本人账号登录后可正常投递。</Banner>}
      {aok && <Banner ok>投递成功！企业会尽快查看你的简历，请留意电话。</Banner>}
      {adup && <Banner>你已投递过该岗位，无需重复投递。</Banner>}
      ...
      {matched.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-border bg-background p-8 text-center text-[13px] text-muted-foreground">
          ...
          {others.length > 0 && <> 或 <Link href="/dashboard/practitioner/hire?all=1" className="text-brand">查看全部 {others.length} 个</Link></>}。
        </div>
      ) : (
        <div className="space-y-3">
          {matched.map((m) => <HireCard key={m.job.id} m={m} st={appliedMap.get(m.job.id)} />)}
        </div>
      )}
        ...
        showAll ? (
          <>
            <div className="flex items-center justify-between px-1 mt-6 mb-2">
              ...
              <Link href="/dashboard/practitioner/hire" className="text-[12px] text-brand">收起</Link>
            </div>
            <div className="space-y-3">
              {others.map((m) => <HireCard key={m.job.id} m={m} st={appliedMap.get(m.job.id)} dim />)}
            </div>
          </>
        ) : (
          <Link href="/dashboard/practitioner/hire?all=1" className="mt-4 block rounded-2xl border border-border bg-background p-3.5 text-center text-[13px] text-brand active:scale-[0.99] transition-transform">
      ...
      <Link href="/ai/hr" className="mt-3 block rounded-3xl bg-foreground text-background p-5 active:scale-[0.99] transition-transform relative overflow-hidden">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[14px] font-semibold">AI 小才 · 优化简历 / 模拟面试</div>
            <div className="text-[11px] text-background/70 mt-0.5">长期岗位投递前先问问 TA</div>
          </div>
          <ChevronRight className="h-4 w-4" />
        </div>
      </Link>
...
function HireCard({ m, st, dim }: { m: JobMatch; st?: string; dim?: boolean }) {
  const j = m.job;
  const pay = `¥${j.daily}${j.dailyMax && j.dailyMax > j.daily ? `-${j.dailyMax}` : ""}`;
  return (
    <div className={`rounded-3xl border border-border bg-background p-4 relative overflow-hidden ${dim ? "opacity-90" : ""}`}>
      <span className={`absolute left-0 top-0 h-1 w-full ${dim ? "bg-border" : "bg-cat-design"}`} />
      <div className="flex items-center gap-1.5 mb-2 flex-wrap">
        <Badge tone="design">{j.kind}</Badge>
        <Badge tone="brand">{j.openings} 人</Badge>
        <span className="inline-flex items-center gap-0.5 text-[10px] text-accent-tea ml-auto"><ShieldCheck className="h-2.5 w-2.5" /> 协会会员企业</span>
      </div>
      ...
      {(m.reasons.length > 0 || (dim && m.gaps.length > 0)) && (
      ...
      <div className="mt-3 flex items-center gap-2 text-[11px] text-muted-foreground flex-wrap">
        ...
        {j.edu && <><span>·</span><span className="inline-flex items-center gap-0.5"><GraduationCap className="h-2.5 w-2.5" />{j.edu}</span></>}
        ...
        {st ? (
          ...
          <form action={applyHireAction}>
            ...
            <button className="h-10 px-4 rounded-full bg-foreground text-background text-[12px] font-medium inline-flex items-center gap-1 active:scale-95 transition-transform">
              投递简历 <ArrowUpRight className="h-3 w-3" />
            </button>
          </form>
        )}
      </div>
