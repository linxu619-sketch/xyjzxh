...
const CATS = ["协会公告", "政策解读", "行业新闻", "会员动态", "活动通知"];
...
export function PublishNews() {
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-foreground/40 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setOpen(false)}>
      <form action={createNewsAction} onClick={(e) => e.stopPropagation()} className="w-full sm:max-w-lg bg-background rounded-t-3xl sm:rounded-3xl border border-border p-5 md:p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[16px] font-semibold">发布新闻 / 公告</h3>
        ...
        <div className="space-y-3">
          <Field label="标题" required><input name="title" required placeholder="如：关于开展年度资质年检的通知" className={INPUT} /></Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="分类"><select name="category" defaultValue="协会公告" className={INPUT}>{CATS.map((c) => <option key={c}>{c}</option>)}</select></Field>
