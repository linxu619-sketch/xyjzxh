...
export default async function CustomerInsurance({ searchParams }: { searchParams: Promise<{ claimed?: string; cerr?: string }> }) {
  ...
  return (
    <CustomerShell title="我的保单" subtitle={`${mine.length} 笔投保申请 · ${claims.length} 笔理赔`}>
      {/* 我提交的投保申请（实时，按登录账号） */}
      ...
      <div className="rounded-3xl bg-gradient-to-br from-cat-decor to-[#e6531f] text-white p-5 mb-4 relative overflow-hidden">
        ...
        <div className="relative mt-2 text-[12px] text-white/80 tracking-wider uppercase">已出单保障</div>
        <div className="relative mt-1 text-[36px] font-semibold leading-none">{done} 笔</div>
        <div className="relative mt-2 text-[12px] text-white/80">{mine.length} 笔投保申请 · {mine.length - done} 笔跟进中</div>
      </div>
      ...
      <div className="mb-4 rounded-3xl border border-border bg-background p-5">
        ...
        <form action={submitClaimAction} className="space-y-2.5">
          <select name="policy" className="w-full h-11 rounded-xl border border-border bg-background px-3.5 text-[14px] outline-none focus:border-foreground/30" defaultValue="">
            <option value="" disabled>{mine.length ? "选择出险的投保 / 险种" : "暂无投保申请，可先在线投保"}</option>
            {mine.map((o) => <option key={o.id} value={`${o.product} · ${fmt(o.createdAt)}`}>{o.product} · {fmt(o.createdAt)}</option>)}
          </select>
