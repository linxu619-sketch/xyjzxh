...
type Member = { id: number; name: string; role: string; exp: string };
    ...
    <div className="rounded-2xl border border-border bg-background p-6 md:p-7">
      ...
      {adding && (
        <form action={createMemberAction} className="rounded-xl border border-border bg-surface/40 p-4 mb-5 space-y-3">
          ...
          <div>
            <div className="text-[12px] font-medium mb-1.5">资历 / 简介</div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={() => setAdding(false)} className="h-10 px-4 rounded-full text-[13px] text-muted-foreground hover:text-foreground">取消</button>
          </div>
        </form>
      )}
