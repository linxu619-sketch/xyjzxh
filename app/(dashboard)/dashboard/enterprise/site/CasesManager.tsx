    ...
    <div className="rounded-2xl border border-border bg-background p-6 md:p-7">
      ...
      {adding && (
        <form action={createCaseAction} className="rounded-xl border border-border bg-surface/40 p-4 mb-5 space-y-4">
          ...
          <MultiUpload label="案例图片（1-10 张，第一张为封面）" hint="支持多选，横版 4:3 效果最佳" max={10} onChange={setImages} />
          ...
          <div className="flex items-center gap-3">
            <button type="submit" disabled={images.length === 0} className="h-10 px-5 rounded-full bg-accent-tea text-white text-[13px] font-medium inline-flex items-center gap-1.5 disabled:opacity-50">
              <Plus className="h-3.5 w-3.5" /> 保存案例
            </button>
            <button type="button" onClick={() => { setAdding(false); setImages([]); }} className="h-10 px-4 rounded-full text-[13px] text-muted-foreground hover:text-foreground">取消</button>
            {images.length === 0 && <span className="text-[11px] text-muted-foreground">先上传至少 1 张图后可保存</span>}
          </div>
        </form>
      )}
