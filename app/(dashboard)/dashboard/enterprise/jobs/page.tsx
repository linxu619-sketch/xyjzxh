...
export const metadata = { title: "用工派单 · 企业工作台" };
...
export default async function JobsPage({ searchParams }: { searchParams: Promise<{ f?: string; jok?: string; jerr?: string }> }) {
  ...
  const all = effectiveEnterpriseId(session) ? listJobsByEnterprise(effectiveEnterpriseId(session)!, "gig") : [];
  ...
  return (
    <EnterpriseShell title="用工派单" subtitle={`零工 / 散工 · 日薪结算 · 在招 ${openCount} · 累计报名 ${totalApps}`} actions={<PostJobForm />}>
      {jok && <div className="mb-5 rounded-2xl border border-accent-tea/30 bg-[#e6f7f1] text-accent-tea p-4 flex items-center gap-3"><CheckCircle2 className="h-5 w-5 shrink-0" /><div className="text-[13px]"><b>用工已发布！</b>对应工种的从业者可在「找活」看到并报名。</div></div>}
      ...
      <StatFilters
        items={[
          { key: "open", label: "在招用工", value: openCount, color: "text-cat-build", href: href("open"), active: active === "open" },
          { key: "closed", label: "已结束", value: closedCount, color: "text-muted-foreground", href: href("closed"), active: active === "closed" },
          { key: "apps", label: "累计报名", value: totalApps, color: "text-accent-tea" },
      ...
      <div className="rounded-2xl border border-border bg-background overflow-hidden">
        ...
        {list.length === 0 ? (
          <div className="px-5 py-16 text-center text-[13px] text-muted-foreground">
            {active ? "没有该状态的用工。" : "还没有用工派单。点右上「发布用工」发布第一个，对应工种的从业者即可在「找活」报名。招聘长期岗位请去「招聘岗位」。"}
          </div>
        ) : (
          <>
