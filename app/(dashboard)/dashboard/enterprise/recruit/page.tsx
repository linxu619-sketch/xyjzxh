...
import { PostRecruitForm } from "./PostRecruitForm";
import { effectiveEnterpriseId } from "@/lib/dashboard/preview";

export const metadata = { title: "招聘岗位 · 企业工作台" };

const FILTERABLE: JobStatus[] = ["open", "closed"];

export default async function RecruitPage({ searchParams }: { searchParams: Promise<{ f?: string; jok?: string; jerr?: string }> }) {
  const { f, jok, jerr } = await searchParams;
  const session = await getSession();
  const all = effectiveEnterpriseId(session) ? listJobsByEnterprise(effectiveEnterpriseId(session)!, "hire") : [];
  ...
  const base = "/dashboard/enterprise/recruit";
  const href = (st: JobStatus) => (active === st ? base : `${base}?f=${st}`);
  const pay = (j: { daily: number; dailyMax: number | null }) => `¥${j.daily}${j.dailyMax && j.dailyMax > j.daily ? `-${j.dailyMax}` : ""}/月`;

  return (
    <EnterpriseShell title="招聘岗位" subtitle={`长期雇佣 · 月薪 · 在招 ${openCount} 岗 · 累计投递 ${totalApps} 份`} actions={<PostRecruitForm />}>
      {jok && <div className="mb-5 rounded-2xl border border-accent-tea/30 bg-[#e6f7f1] text-accent-tea p-4 flex items-center gap-3"><CheckCircle2 className="h-5 w-5 shrink-0" /><div className="text-[13px]"><b>招聘已发布！</b>符合条件的个人会员可在「找工作」看到并投递。</div></div>}
      {jerr && <div className="mb-5 rounded-2xl border border-cat-decor/30 bg-cat-decor-soft text-cat-decor p-4 flex items-center gap-3"><AlertCircle className="h-5 w-5 shrink-0" /><div className="text-[13px]">发布失败：请填写岗位标题与职位。</div></div>}
      ...
      <div className="rounded-2xl border border-border bg-background overflow-hidden">
        ...
        {list.length === 0 ? (
          <div className="px-5 py-16 text-center text-[13px] text-muted-foreground">
            {active ? "没有该状态的岗位。" : "还没有招聘岗位。点右上「发布招聘」招专业人才（设计师/项目经理/监理等，月薪）。临时零工请去「用工派单」。"}
          </div>
        ) : (
          <>
            <div className="hidden md:grid grid-cols-[1.8fr_1fr_1.3fr_0.8fr_auto] gap-3 px-5 py-2.5 border-b border-border text-[11px] text-muted-foreground tracking-wider">
              <span>岗位</span><span>职位</span><span>月薪 / 名额</span><span>投递</span><span className="text-right">状态</span>
            </div>
                    ...
                    <Link href={`/dashboard/enterprise/jobs/${j.id}`} className="grid grid-cols-[1fr_auto] md:grid-cols-[1.8fr_1fr_1.3fr_0.8fr_auto] gap-3 items-center px-5 py-3.5 text-[13px] hover:bg-surface transition-colors active:scale-[0.99]">
                      <span className="min-w-0 inline-flex items-center gap-2">
                        ...
                        <span className="min-w-0">
                          <span className="font-medium truncate flex items-center gap-1.5">{j.title}</span>
                          <span className="md:hidden text-[11px] text-muted-foreground truncate block">{j.kind} · {pay(j)} · 录用{hired}/{j.openings} · {apps}投递</span>
                        </span>
                      </span>
                      <span className="hidden md:block text-muted-foreground truncate">{j.kind}</span>
                      <span className="hidden md:block text-muted-foreground">{j.district || "信阳"} · {pay(j)} · 录用 {hired}/{j.openings}</span>
